...
dataloader = ImageDataset("./data/%s"%opt.dataset_name, transforms=transforms_).set_attrs(
    batch_size=opt.batch_size,
    shuffle=True,
    num_workers=opt.n_cpu,
)
val_dataloader = ImageDataset("./data/%s"%opt.dataset_name, transforms=transforms_, mode="test").set_attrs(
    batch_size=10,
    shuffle=True,
    num_workers=1,
)
...
def sample_images(epoch):
    ...
    real_B, real_A, _ = next(iter(val_dataloader))
    fake_B = generator(real_A)
    img_sample = np.concatenate([real_A.data, fake_B.data, real_B.data], -2)# [10,3,256,256,] 
    save_image(img_sample, "images/%s/%s.png" % (opt.dataset_name, epoch), nrow=5)
...
def train():
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, (real_B, real_A,_) in enumerate(dataloader):# real_B彩色, real_A标签 :[8,3,256,256,]
            # Adversarial ground truths
            valid = jt.ones([real_A.shape[0], 1]).stop_grad()# [8,1,]
            fake = jt.zeros([real_A.shape[0], 1]).stop_grad()# [8,1,]

            # ------------------
            #  Train Generators
            # ------------------
            # GAN loss
            fake_B = generator(real_A)# [8,3,256,256,]根据标签生成彩图
            pred_fake = discriminator(fake_B, real_B)# 对生成图片和训练集彩图进行判别
            loss_GAN = criterion_GAN(pred_fake, True)# [8,1,16,16,] 计算损失值
            # Pixel-wise loss
            ...
            # ---------------------
            # Real loss
            pred_real = discriminator(real_B, real_A)# 训练集中的彩色图片和标签
            loss_real = criterion_GAN(pred_real, True)
            # Fake loss
            pred_fake = discriminator(fake_B.detach(), real_A)# 生成彩色图片和训练集标签
